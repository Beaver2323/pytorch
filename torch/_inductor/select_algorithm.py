...
class AutotuneArgs:
    ...
    def verify(self, benchmark_tensors: BenchmarkTensors | None = None, **kwargs):
        """Verify the correctness of the benchmarking results"""

        tensors = benchmark_tensors or self.extern
        torch.testing.assert_close(tensors.output_tensor, self.expected, **kwargs)
...
class AlgorithmSelectorCache(PersistentCache):
    ...
    def get_inputs(
        ...
        for choice in choices:
            ...
            try:
                ...
                log.debug(
                    "Failed to compute layout key for choice=%s: %s",
        ...
        if VERIFY:
            ...
            if override is not None:
                ...
                verify_out = override.output_tensor
            if verify_out is None:
                raise RuntimeError("Verification requires an output tensor")
            choices[0].benchmark(*verify_inputs, out=verify_out)
            expected = verify_out.clone()
    ...
    def _materialize_inputs(
        ...
        def addmm_unique_example_inputs_extern():
            ...
            for i, (input_node, extern_node) in enumerate(
                zip(input_nodes, extern_input_nodes)
            ):
                ...
                if i == 0:
                    ...
                    global_tensor = global_tensor[0].contiguous()
                additional_example_inputs[extern_name] = global_tensor
    ...
    def benchmark_choice(
        ...
        try:
            ...
            if VERIFY and autotune_args.expected is not None:
                autotune_args.verify(benchmark_tensors, **VERIFY)
            return result
        finally:
