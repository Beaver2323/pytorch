...
// unwrap the shared_ptr so `_push`/`_pop`/`_from_meta_and_device` can operate on
// an explicitly-passed mode rather than the ambient TLS state.
...
cpp_fake_mode_save_stack() {
  static thread_local std::
      vector<std::pair<std::shared_ptr<c10::FakeTensorMode>, bool>>
          stack;
  return stack;
}
...
:func:`torch.set_num_threads` onto the new thread.
  ...
  py_module.def(
      ...
      [](const at::Tensor& fake, const at::Tensor& real) {
        fake.unsafeGetTensorImpl()->set_real_tensor(
            std::make_shared<at::Tensor>(real));
      },
      py::arg("fake"),
      py::arg("real"));
  ...
  py_module.def(
      ...
      [](const py::object& converter,
        ...
        // handlers (PyInterpreter.cpp) observe the same single identity. The
        // mode is NOT registered in TLS here: its Python object owns it via the
        // returned capsule and installs it into TLS only for the duration of a
        // `with cpp_mode:` scope (see _push/_pop below).
